 ...
 *
 * <p>
 * curl 'https://duckduckgo.com/?q=site:linkedin.com "Jon
 * Snow"&t=newext&atb=v300-1'
 */
@Service
					...
					try {
						...
						scrappedData.stream().forEach(el -> {
							if (el.toString().contains(GRAPH)) {
								String payload = el.toString().substring(el.toString().indexOf("{"),
										el.toString().lastIndexOf("}") + 1);
								logger.info(payload);

								try {
									LinkedInPerson linkedInPerson = objectMapper.readValue(payload,
											LinkedInPerson.class);
									Optional<Graph> person = linkedInPerson.getGraph().stream()
											.filter(gp -> gp.getType().equals("Person")).findFirst();
