 ...
 *
 * <p>curl 'https://duckduckgo.com/?q=site:linkedin.com "Jon Snow"&t=newext&atb=v300-1'
 */
@Service
public class LinkedInOps implements Operations {
  ...
  public void runSearch(String query) {
    ...
    try {
      ...
      if (doc.getElementById("links") != null
          && doc.getElementById("links").getElementsByClass("results_links") != null) {
                    ...
                    el -> {
                      if (el.toString().contains(GRAPH)) {
                        String payload =
                            el.toString()
                                .substring(
                                    el.toString().indexOf("{"), el.toString().lastIndexOf("}") + 1);
                        logger.info(payload);

                        try {
                          LinkedInPerson linkedInPerson =
                              objectMapper.readValue(payload, LinkedInPerson.class);
                          Optional<Graph> person =
                              linkedInPerson.getGraph().stream()
                                  .filter(gp -> gp.getType().equals("Person"))
                                  .findFirst();

                          if (person.isPresent()) {
                            lidaService.create(
                                Utilities.buildFrom(person.get(), title.attr("href")));
                          }
                        ...
                        logger.info(
                            "Snippet:"
                                + result.getElementsByClass("result__snippet").first().text());
                      }
                    });
          } catch (IOException e) {
            logger.error(e.getMessage());
